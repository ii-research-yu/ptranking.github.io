...
from ptranking.ltr_adversarial.util.point_sampling import generate_true_docs
...
class IRFGAN_Point(AdversarialMachine):
    ...
    def mini_max_train(self, train_data=None, generator=None, discriminator=None, d_epoches=1, g_epoches=1, dict_buffer=None, num_samples_per_query=3, single_step=True):
        if single_step:
            stop_training = self.train_discriminator_generator_single_step(train_data=train_data, generator=generator, discriminator=discriminator, dict_buffer=dict_buffer, num_samples_per_query=num_samples_per_query)
